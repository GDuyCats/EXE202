...
import { updateProduct } from './ProductService'; // Adjust the path as necessary

const UpdateProductModal = ({ productId, existingProductData, setProducts, onClose }) => {
  ...
  useEffect(() => {
    if (existingProductData) {
      setFormData({
        ...existingProductData,
        productMaterials: existingProductData.productMaterials ?? [{
          detail: '',
          materialId: '',
          material: {
            childCategoryId: '',
        ...
        images: existingProductData.images ?? [{
          file: null,
          thumbnail: false
        }]
      });
  ...
  const handleSubmit = (e) => {
    ...
    // Log the form data to check its content before sending
    for (let pair of data.entries()) {
      console.log(`${pair[0]}: ${pair[1]}`);
    }

    // Send the form data to the server
    updateProduct(productId, data, setProducts);
    onClose();
  };
